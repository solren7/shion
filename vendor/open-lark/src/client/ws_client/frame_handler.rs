...
impl FrameHandler {
    ...
    async fn process_event(
        ...
        // Keep a bounded copy so a deserialization failure can report the
        // offending JSON instead of just a serde column offset.
        let snippet: String =
            String::from_utf8_lossy(&payload[..payload.len().min(2000)]).into_owned();
        ...
        match event_handler.do_without_validation(payload) {
            ...
            Err(err) => {
                error!("Failed to handle event: {err:?}; payload: {snippet}");
                NewWsResponse::error()
            }
        }
    }
